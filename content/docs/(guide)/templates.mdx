...
```sql
-- Step 1: Build the view that produces the desired rows
...
-- Step 2: Tag the view as a template (optionally name its preferred target table)
...
-- Step 3: Grant access directly to the roles that should see this template
REVOKE ALL ON hostel.monthly_payment_template FROM authenticated, service_role;
GRANT SELECT ON hostel.monthly_payment_template TO "x-admin";
```
...
- **[Row Actions](/resource/actions)** — The same `type`-tagged pattern, applied to per-row functions instead of bulk-insert views
