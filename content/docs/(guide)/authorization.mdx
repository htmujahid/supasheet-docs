...
- `public.tasks:audit` - Read the audit trail for a record
- `public.tasks:comment` - Read and post comments on a record
...
Supasheet ships with three roles defined on the `supasheet.app_role` enum:

#### `user` (Default)
- Automatically assigned to new sign-ups via the `on_auth_user_created` trigger
- Has no permissions by default — grant access explicitly for each resource
- Recommended for end-users of your application

#### `admin`
- An intermediate role with no built-in permissions
- Reserved for application admins; grant the resource permissions that match your business rules

#### `x-admin`
- The Supasheet super-admin role
- Seeded with permissions for the entire `supasheet` schema (users, roles, permissions, audit logs, notifications, etc.)
- Required to manage roles, permissions, and other admins through the UI under **Core**

<Callout type="warn">
`x-admin` is the only role that can manage other roles and permissions. Always have at least one user with this role.
</Callout>

### Special-Case Permissions

Beyond the standard CRUD operations, Supasheet recognises two additional per-table permissions:

- `schema.table:audit` — required to view the per-record audit trail on the detail page
- `schema.table:comment` — required to read and post comments on a record
...
insert into supasheet.user_roles (user_id, role) values
  ('user-uuid-here', 'user'),
...
To change what role new users get automatically, uncomment the trigger function in accounts.sql migration:
...
<Callout type="info">
Supasheet's meta schema is kept in sync automatically by event triggers (`table_creation_trigger`, `comment_trigger`, `enum_alteration_trigger`, etc.) defined in `supabase/migrations/00000000000000_meta.sql`. You do **not** need to manually refresh `supasheet.tables` or `supasheet.columns` after running DDL.
</Callout>
...
- **[Complete Example](/guide/complete-example)** - See full workflow
- **[Database Schema](/guide/database-schema)** - Understand schema organization
- **[User Management](/guide/user-management)** - Learn about accounts
