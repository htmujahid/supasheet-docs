...
2. Add JSON metadata comment with `type: "chart"` and `chart_type`
3. Grant access on the view — revoke all, then `grant select` directly to the roles that should see the chart (no enum, no permissions table)

## Chart Types

Supasheet's `chart_type` metadata value supports these types:
...
grant select on public.task_trend_area to "user";
```
...
grant select on public.task_priority_bar to "user";
```
...
grant select on public.task_completion_line to "user";
```
...
grant select on public.task_metrics_radar to "user";
```

### Additional Patterns

The five `chart_type` values above cover the primitive chart shapes. You can combine them with different query shapes to get more specific presentations:
...
Descriptions and formatting hints belong in the JSON `comment` on the view, alongside `type` and `chart_type` — not as extra literal columns in the `SELECT` list:
...
COMMENT ON VIEW public.monthly_revenue IS
  '{"type": "chart", "name": "Monthly Revenue", "description": "Monthly revenue trend for the last 12 months", "chart_type": "line", "format": "currency"}';
```
...
For complex or slow queries. See [Materialized Views](/resource/materialized-views) for how Supasheet treats these as a resource type in their own right:
...
- **[Dashboards](/dashboards)** - Combine charts into dashboards
- **[Reports](/reports)** - Generate detailed reports
- **[Database Schema](/database-schema)** - Learn more about the charts schema
