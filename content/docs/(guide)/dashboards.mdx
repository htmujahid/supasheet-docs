...
2. Add JSON metadata comment with `type: "dashboard_widget"` and `widget_type`
3. Revoke default grants, then grant `select` directly to the roles that should see the widget
...
grant select on public.task_summary to "user";

-- Step 3: Add metadata comment
...
grant select on public.task_completion_rate to "user";
...
grant select on public.tasks_by_status to "user";
...
grant select on public.task_urgent_count to "user";
...
grant select on public.task_list_simple to "user";
...
grant select on public.task_list_detailed to "user";
...
For expensive queries. See [Materialized Views](/resource/materialized-views) for how Supasheet treats these as a resource type in their own right:
...
- **[Charts](/charts)** - Add visualizations to your dashboards
- **[Reports](/reports)** - Generate detailed reports
- **[Authorization](/authorization)** - Control dashboard access
