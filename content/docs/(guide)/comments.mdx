...
<Callout type="info">
Don't confuse this with the `tabs` metadata field described in [Metadata](/guide/resource/metadata#tabs) — that field only filters which **related-table** tabs appear on the detail page (foreign key relationships). Comments and the audit log are separate, permission-gated pages, not metadata-configured tabs.
</Callout>
...
See **[Notifications](/guide/notifications)** for the full API.
...
- **[Notifications](/guide/notifications)** — Notify users when new comments land
- **[Audit Logs](/guide/audit-logs)** — Pair comments with a full change history
- **[Authorization](/guide/authorization)** — Grant `:comment` per role
