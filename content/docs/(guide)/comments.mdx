...
<Callout type="info">
Don't confuse this with the `tabs` metadata field described in [Metadata](/resource/metadata#tabs) — that field only filters which **related-table** tabs appear on the detail page (foreign key relationships). Comments and the audit log are separate, access-gated pages, not metadata-configured tabs.
</Callout>
