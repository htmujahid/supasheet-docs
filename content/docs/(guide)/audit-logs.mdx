...
<Callout type="info">
Don't confuse this with the `tabs` metadata field documented in [Metadata](/resource/metadata#tabs) — that field only filters which **related-table** tabs show on the detail page (foreign key relationships). It has no effect on audit or comment access, which are controlled purely by permissions.
</Callout>
...
- **[Authorization](/authorization)** — How `:audit` slots into the permission model
