...
type TableMetadata = {
  ...
  description?: string
  icon?: string
  collapsible_group?: string        // sidebar collapsible section this resource is grouped under
  singleton?: boolean
  inline_form?: boolean
  query?: {
  ...
  detail?: {                         // table-only — views have no detail page
    header?: { title?: string; badges?: string[] }
    tabs?: string[]
  }
}
```
...
Lists the fields shown in the abbreviated "quick create" form (e.g. the inline creator opened from a lookup or a kanban column's "+" button) — everything else is left to its default value and can be filled in later.
...
```typescript
// source_column is always a field on this table's own form;
// target_column is always a column on the lookup target table.
type LookupFillRule   = { source_column: string; target_column: string }
type LookupFilterRule = { source_column: string; target_column: string }
...
COMMENT ON TABLE store.orders IS '{
  "fields": {
    "lookups": {
      "customer_id": {
        "fill": [
          {"source_column": "shipping_address", "target_column": "default_address"},
          {"source_column": "shipping_city",    "target_column": "default_city"}
        ]
      },
      "product_id": {
        "filter": [
          {"source_column": "warehouse_id", "target_column": "warehouse_id"}
        ]
      }
    }
  }
...
Table-only — views have no detail page. Configures the detail page's heading and which related-table tabs it shows.
...
`header.title` names a column whose value renders as the page heading (falls back to the record's primary key when empty). `header.badges` names columns rendered as badges next to the heading.

`tabs` filters which **related-table tabs** appear on the record detail page. By default — when `tabs` is omitted — every one-to-one, one-to-many, and many-to-many relationship Supasheet detects is shown as its own tab alongside the main "Detail" tab. Set `tabs` to a list of relationship names to show only those and hide the rest. Each string must match the relationship's name/embed key (the foreign table's PostgREST embed name), not a column or arbitrary label. The main "Detail" tab is always shown regardless of this setting.

<Callout type="info">
`tabs` has nothing to do with the Audit Log or Comments features — those are separate, grant-gated pages (audit via a fixed `SELECT` grant on `supasheet.audit_logs`; comments via the resource's own `SELECT` grant) linked from the record's actions menu, not tabs controlled by this field. See [Audit Logs](/audit-logs) and [Comments](/comments). Custom, function-backed buttons in that same actions menu are configured separately — see [Row Actions](/resource/actions).
</Callout>
...
type EnumColumnMetadata = ColumnMetadata & {
  progress?: boolean
  icon_only?: boolean  // show only the icon (in a tooltip), not the text label
  values?: {
    [value: string]: {
      icon?: string
...
type FileColumnMetadata = ColumnMetadata & {
  ...
  max_files?: number   // Maximum file count (FILE / array)
...
- **[Row Actions](/resource/actions)** — Custom operations that reuse this same `FieldCondition` shape for visibility
