...
<Callout type="info">
`tabs` has nothing to do with the Audit Log or Comments features — those are separate, permission-gated pages (`schema.table:audit` / `schema.table:comment`) linked from the record's actions menu, not tabs controlled by this field. See [Audit Logs](/audit-logs) and [Comments](/comments).
</Callout>
