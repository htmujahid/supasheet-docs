...
Available filter operators depend on the column's data type — text, number, date, boolean, select, multi-select, and UUID columns each get their own operator set (contains, is, is before/after, has any of, and so on). See **[Operators by Column Type](/resource/query#operators-by-column-type)** for the exact list per type.
...
```sql
CREATE TABLE products (
  id UUID DEFAULT extensions.uuid_generate_v4(),  -- Auto-generated
...
<Callout type="warn">
This only works when the referenced table lives in the **same schema** as the one being queried — PostgREST can't embed a foreign key that points to a different schema (e.g. a `store.orders.user_id` FK pointing at `supasheet.users`). If you need to reference a table from another schema, create a same-named replica view (`with (security_invoker = true)`) in your own schema first. See [Cross-Schema Joins](/resource/query#cross-schema-joins) for the exact pattern.
</Callout>
...
<Callout type="info">
These are the built-in View / Edit / Delete actions every resource gets for free. You can also define your own custom, permission-gated actions (cancel, publish, duplicate, ...) as tagged SQL functions — see **[Row Actions (custom)](/resource/actions)**.
</Callout>
...
Every resource supports **📤 CSV export** and **📥 CSV import** — no configuration needed.

**Export** writes the currently visible columns to a CSV file, using each column's display `name` (see [Metadata](/resource/metadata)) as the header — not the raw database column name. You can export everything in the current view or just the selected rows.

**Import** parses a CSV client-side, matches its header row against the table's columns by name, and inserts in batches of 500 rows at a time. Each value is coerced to the target column's type (numbers, booleans, dates, etc.) before insert, and unmatched columns in the CSV are ignored rather than rejected.

<Callout type="warn">
Import respects the same RLS policies and form validation as manual creation — rows that fail a `NOT NULL`, `CHECK`, or RLS policy are skipped, not silently defaulted.
</Callout>
...
CREATE TABLE orders (
  id UUID PRIMARY KEY DEFAULT extensions.uuid_generate_v4(),
  customer_id UUID REFERENCES supasheet.users(id),
...
- **[Metadata](/resource/metadata)** - Configure icons, visibility, columns, and views
- **[Query Configuration](/resource/query)** - Set default sorting and filters
- **[Views](/resource/views)** - Multiple view types (Sheet, Kanban, Calendar, Gallery, List, Tree)
- **[Row Actions](/resource/actions)** - Custom, permission-gated operations beyond CRUD
- **[Data Types](/resource/data-types)** - Use custom data types
- **[Authorization](/authorization)** - Secure your tables with RLS
