...
-- With size limit
COMMENT ON COLUMN tasks.document IS '{"accept":".pdf","max_size":10485760,"max_files":1}';
```
...
| `max_size` | number | `5242880` (5MB) | Maximum file size in bytes |
| `max_files` | number | `1` | Maximum number of files allowed |
...
COMMENT ON COLUMN products.cover IS '{"accept":"image/*","max_size":2097152}';
COMMENT ON COLUMN products.logo IS '{"accept":"image/*","max_size":1048576}';
```
...
- **[Query Configuration](/resource/query)** - Configure default queries
- **[Views](/resource/views)** - Multiple view types for data
