...
The comment accepts the full [table metadata](/guide/resource/metadata) shape on top of `based_on` — `views`, `filter_presets`, `fields.sections`, `query`, `tabs`, and so on. Column hints refer to the view's columns.
