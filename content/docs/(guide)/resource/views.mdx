...
Inline editing uses the field sections defined in the [metadata `fields.sections`](/resource/metadata) configuration.
...
- **Use `badge` thoughtfully.** Pair it with the [enum metadata](/resource/metadata#enum-columns) to colour-code statuses and priorities.
...
- **[Metadata](/resource/metadata)** — Configure sections, filter templates, enum styling
- **[Query Configuration](/resource/query)** — Default sorts, filters, and joins
- **[Data Types](/resource/data-types)** — Custom types that power covers and badges
