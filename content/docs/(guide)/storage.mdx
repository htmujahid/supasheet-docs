...
COMMENT ON COLUMN store.products.image   IS '{"accept": "image/*", "max_size": 5242880, "max_files": 1}';
COMMENT ON COLUMN store.products.manuals IS '{"accept": ".pdf",   "max_size": 10485760, "max_files": 5}';
```
...
COMMENT ON COLUMN blog.authors.avatar IS '{"max_size": 2097152}';
```
...
Useful constants when setting `max_size`:
...
<Callout type="info">
Supabase Storage also enforces a global `file_size_limit` per project. Make sure your `max_size` is below the project limit, otherwise uploads will fail at the storage layer.
</Callout>
