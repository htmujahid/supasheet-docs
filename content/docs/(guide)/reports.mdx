...
2. Add JSON metadata comment with `type: "report"`
3. Grant permissions on the view (revoke all, then grant select to the roles that should see it)
...
-- Step 3: Grant permissions (revoke all, then grant select to the roles that should see it)
revoke all on public.task_report from authenticated, service_role;
grant select on public.task_report to "user";
```

**Report Comment Metadata:**
- `type` - Must be "report"
...
-- Refresh daily
REFRESH MATERIALIZED VIEW public.monthly_stats;
```
...
- **[Dashboards](/dashboards)** - Create visual dashboards
- **[Charts](/charts)** - Add visualizations
- **[Authorization](/authorization)** - Secure your reports
