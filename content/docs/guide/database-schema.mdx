...
<Callout type="warn">
**Important**: Always use `supasheet.users(id)` for user relationships, not `auth.users(id)`. See [User Management](/docs/guide/user-management) for details.
</Callout>
...
- `supasheet.users` - User accounts (auto-created on signup)
...
SELECT
  ...
  t.*
FROM tasks t
JOIN supasheet.users a ON t.account_id = a.id;
