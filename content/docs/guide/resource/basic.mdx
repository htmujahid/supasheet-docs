...
## Table Configuration via Comments

You can customize how tables are displayed in Supasheet using JSON metadata in table comments.

### Custom Icon

Set a custom icon for your table using Lucide icon names:
...
**Available Icons:** Any icon name from [Lucide Icons](https://lucide.dev/icons/) (e.g., `"ListTodo"`, `"Package"`, `"Users"`, `"ShoppingCart"`)

### Display Mode

Control whether a table appears in the resources list:

```sql
-- Hide table from resources (default: "block")
COMMENT ON TABLE internal_logs IS '{"display": "none"}';

-- Show table in resources
COMMENT ON TABLE products IS '{"display": "block"}';
```

**Options:**
- `"block"` - Table appears in resources sidebar (default)
- `"none"` - Table is hidden from UI but still accessible via direct URL
...
This configuration:
- Uses folder icon in the sidebar
- Shows table in resources list

## Next Steps

- **[Query Configuration](/docs/guide/resource/query)** - Set default sorting and filters
- **[Views](/docs/guide/resource/views)** - Multiple view types (Sheet, Kanban, Calendar, Gallery)
