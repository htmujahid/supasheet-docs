...
Available filter operators depend on the column's data type — text, number, date, boolean, select, multi-select, and UUID columns each get their own operator set (contains, is, is before/after, has any of, and so on). See **[Operators by Column Type](/guide/resource/query#operators-by-column-type)** for the exact list per type.
...
**Export** writes the currently visible columns to a CSV file, using each column's display `name` (see [Metadata](/guide/resource/metadata)) as the header — not the raw database column name. You can export everything in the current view or just the selected rows.
...
- **[Metadata](/guide/resource/metadata)** - Configure icons, visibility, columns, and views
- **[Query Configuration](/guide/resource/query)** - Set default sorting and filters
- **[Views](/guide/resource/views)** - Multiple view types (Sheet, Kanban, Calendar, Gallery, List, Tree)
- **[Data Types](/guide/resource/data-types)** - Use custom data types
- **[Authorization](/guide/authorization)** - Secure your tables with RLS
