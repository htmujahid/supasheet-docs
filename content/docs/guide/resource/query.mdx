...
This will:
- Join the `accounts` table on `user_id`
...
If you need to join a table from another schema (e.g., `supasheet.users`), create a view in your working schema:

```sql
-- Create a view in the public schema that references supasheet.users
...
- **[Views](/docs/guide/resource/views)** - Configure six view types (Sheet, Kanban, Calendar, Gallery, List, Tree)
