...
type TableMetadata = {
  ...
  primaryItem?: string
  items?: MetaViewItem[]            // Kanban | Calendar | Gallery | List | Tree
...
| `"none"` | Hidden from the sidebar — still reachable by direct URL or as a child via `inlineForm` |
...
`items` declares the alternate views (Kanban, Calendar, Gallery, List, Tree). `primaryItem` is the `id` of the view to load by default. See **[Views](/docs/guide/resource/views)** for the per-view configuration reference.
...
If `primaryItem` is omitted, the default sheet view is shown.
...
COMMENT ON TABLE store.products IS '{
  "metadataColumns": ["created_at", "updated_at", "created_by"]
}';
```
...
COMMENT ON TABLE desk.tasks IS $$
{
  ...
  "items": [
    ...
    {"id": "calendar", "name": "Task Schedule",     "type": "calendar",
     "title": "title", "startDate": "due_date", "badge": "status"}
  ],
  "filterTemplates": [
    {"id": "open",      "name": "Open (not completed)",
     "filters": [{"id": "status", "value": ["pending", "in_progress"],
