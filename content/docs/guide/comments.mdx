...
## Showing the Comment Tab in the UI

Add `comment` to the `tabs` array in the resource's metadata:
...
Users with `desk.tasks:comment` will see a **Comments** tab on each task detail page where they can browse the thread, post new comments, and edit / delete their own.
...
See **[Notifications](/docs/guide/notifications)** for the full API.
...
- **[Notifications](/docs/guide/notifications)** — Notify users when new comments land
- **[Audit Logs](/docs/guide/audit-logs)** — Pair comments with a full change history
- **[Authorization](/docs/guide/authorization)** — Grant `:comment` per role
