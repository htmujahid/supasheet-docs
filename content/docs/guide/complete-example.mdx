...
<Callout type="warn">
**This step is required, not optional.** The table you're about to create lives in `public`, but the canonical user table is `supasheet.users` — a different schema. PostgREST cannot embed a foreign key across schemas, so without this replica, the `query.join` you'll add to the table's own metadata (Step 4) and the `user_tasks`/`task_report` views (Steps 6 and 9) will silently fail to resolve. See [Cross-Schema Joins](/guide/resource/query#cross-schema-joins) for why.
</Callout>
...
<Step>
### Step 11: Grant Permissions to Roles
...
2. **Permissions First**: Always add permissions to `supasheet.app_permission` enum before creating resources
3. **RLS is Required**: Every table must have RLS policies that check permissions using `supasheet.has_permission()`
4. **Revoke Then Grant**: Always revoke all permissions first, then explicitly grant what's needed
...
6. **Grant to Roles**: Assign permissions to roles in `supasheet.role_permissions` table
...
- **[Authorization](/guide/authorization)** - Understanding the permission system
- **[Dashboards](/guide/dashboards)** - Learn more about widget types
- **[Charts](/guide/charts)** - Explore chart types in detail
- **[Reports](/guide/reports)** - Advanced report features
- **[Audit Logs](/guide/audit-logs)** - Viewing and filtering audit logs
