...
alter type supasheet.app_permission add value if not exists 'public.task_completion_rate:select';
alter type supasheet.app_permission add value if not exists 'public.tasks_by_status:select';
alter type supasheet.app_permission add value if not exists 'public.task_urgent_count:select';
...
alter type supasheet.app_permission add value if not exists 'public.task_priority_bar:select';
alter type supasheet.app_permission add value if not exists 'public.task_completion_line:select';
alter type supasheet.app_permission add value if not exists 'public.task_status_pie:select';
alter type supasheet.app_permission add value if not exists 'public.task_metrics_radar:select';
```
...
<Step>
### Step 7: Grant Dashboard Widget Permissions

Grant permissions to roles for the dashboard widgets:
...
**Note:** Dashboard widgets are identified by the JSON metadata in their view comments. The metadata includes:
- `type`: "dashboard_widget"
- `name`: Display name
- `description`: Widget description
- `widget_type`: One of `card_1`, `card_2`, `card_3`, `card_4`, `table_1`, `table_2`
...
<Step>
### Step 9: Grant Chart Permissions

Grant permissions to roles for the charts:
...
**Note:** Charts are identified by the JSON metadata in their view comments. The metadata includes:
- `type`: "chart"
- `name`: Display name
- `description`: Chart description
- `chart_type`: One of `area`, `bar`, `line`, `pie`, `radar`
...
<Step>
### Step 11: Grant Report Permissions

Grant permissions to roles for the reports:
...
**Note:** Reports are identified by the JSON metadata in their view comments. The metadata includes:
- `type`: "report"
- `name`: Display name
