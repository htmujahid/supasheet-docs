...
### `supasheet.users` Table

Every user in Supasheet has an account in the `supasheet.users` table. This is the central user profile table that stores:
...
**Always reference `supasheet.users` instead of `auth.users`** in your application tables.
...
CREATE TABLE tasks (
    ...
    -- ✅ CORRECT: Reference supasheet.users
    account_id UUID REFERENCES supasheet.users(id) ON DELETE CASCADE,

    created_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP
);
```

### Why Use `supasheet.users`?
...
6. **Accounts** - Manage all users in the system (view, create, update, delete) if you have the required permissions: `supasheet.users:select`, `supasheet.users:insert`, `supasheet.users:update`, and `supasheet.users:delete`
