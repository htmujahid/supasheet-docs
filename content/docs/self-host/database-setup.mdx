...
- **Meta layer**: `supasheet.tables`, `columns`, `views`, `materialized_views` — materialized views refreshed manually via `select supasheet.refresh_metadata();` after DDL changes (see [Database Schema](/guide/database-schema#refreshing-the-meta-layer))
- **Users**: `supasheet.users` with triggers to mirror `auth.users`
- **RBAC**: `supasheet.user_roles`, `role_permissions`, plus `app_role` and `app_permission` enums and helpers `has_role()` / `has_permission()`
...
Each migration that adds tables typically does six things — add permissions to the `app_permission` enum, create the table, enable RLS + policies, grant the new permissions to roles, (optionally) attach the audit trigger, and refresh the meta layer:
...
See **[Quickstart](/guide/quickstart)** and **[Complete Example](/guide/complete-example)** for full templates, and **[Examples in the repo](https://github.com/supasheet/supasheet/tree/main/supabase/examples)** for 12 reference domains.
