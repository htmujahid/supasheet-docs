...
- **Meta layer**: `supasheet.tables`, `columns`, `views`, `materialized_views` (with event triggers that auto-sync on DDL)
...
- **Storage buckets**: `public`, `personal`, `uploads`, `account_image` with RLS policies
...
Each migration that adds tables typically does five things — add permissions to the `app_permission` enum, create the table, enable RLS + policies, grant the new permissions to roles, and (optionally) attach the audit trigger:
...
See **[Quickstart](/docs/guide/quickstart)** and **[Complete Example](/docs/guide/complete-example)** for full templates, and **[Examples in the repo](https://github.com/htmujahid/supasheet/tree/main/supabase/examples)** for 12 reference domains.

## Next Steps

- **[Environment Variables](/docs/self-host/environment-variables)** — Configure the SPA
- **[Deployment](/docs/self-host/deployment)** — Build & deploy
